...
n_s = 4754
y = SP.array(list(csv.reader(open('pheno','r'),
...
# load genotypes
X = SP.array(list(csv.reader(open('geno','r'),delimiter='\t'))).astype(float)
...
parents = SP.array(list(csv.reader(open('parents.txt','r'),
                                   delimiter='\t'))).astype(int)

#use sample_idx
split_idx=SP.array(list(csv.reader(open('sample_idx','r'),
                                   delimiter='\t'))).astype(int)
idxmc=[]
idxfc=[]
...
idxm=[x for x in list(range(1,191)) if x not in idxmc]
idxf=[x for x in list(range(1,26)) if x not in idxfc]
print(idxm, idxf)

#random index
##parents=parents[:4754,:]
##idxm=list(range(1,191))
##SP.random.shuffle(idxm)
##idxm=idxm[:5]
##idxf=list(range(1, 26))
...
def train_and_eval1(Xtrain,Xtest,ytrain):
    ns=Xtrain.shape[0]
    idx=list(range(ns))
    ...
    param = {'eta':0.05,'silent':1,
            'subsample':0.5,
           'lambda':0.8}
    model=xgb.train(param, xg_train, 200, [(xg_valid, 'valid')],
                    early_stopping_rounds=20, verbose_eval=10)
    res=model.predict(xgb.DMatrix(Xtest), ntree_limit=model.best_ntree_limit)
    return res.reshape((res.shape[0],1))

def train_and_eval2(Xtrain,Xtest,ytrain):
    ytrain=ytrain.ravel()
...
def train_and_eval(Xtrain,Xtest,ytrain):
    ...
    idx=list(range(ns))
    SP.random.shuffle(idx)
    train_idx=idx[:int(ns*0.8)]
    valid_idx=idx[int(ns*0.8):]
    pred1=train_and_eval1(Xtrain[train_idx],Xtrain[valid_idx],ytrain[train_idx])
    pred2=train_and_eval2(Xtrain[train_idx],Xtrain[valid_idx],ytrain[train_idx])
    preddiff=pred1-pred2
    pred2err=ytrain[valid_idx]-pred2
    t=(LA.norm(preddiff)/LA.norm(pred2err))**2
    if t>1:
        t=1
    if t<0:
        t=0
    pred1=train_and_eval1(Xtrain,Xtest,ytrain)
    pred2=train_and_eval2(Xtrain,Xtest,ytrain)
    return t*pred1+(1-t)*pred2
